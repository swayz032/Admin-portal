...
/** Action types that qualify as finance-related receipts */
const FINANCE_KEYWORDS = ['invoice', 'payment', 'finance', 'billing', 'charge', 'refund', 'subscription'];

function isFinanceReceipt(receipt: TrustReceipt): boolean {
  const actionLower = receipt.action_type.toLowerCase();
  return FINANCE_KEYWORDS.some(kw => actionLower.includes(kw));
}
...
export default function FinanceView() {
  ...
  const financeReceipts = useMemo(
    () => allReceipts.filter(isFinanceReceipt),
    [allReceipts],
  );

  const filteredReceipts = financeReceipts.filter(r => {
  ...
  // Stats
  const totalCount = financeReceipts.length;
  const succeededCount = financeReceipts.filter(r => r.status === 'success').length;
  const failedCount = financeReceipts.filter(r => r.status === 'failed' || r.status === 'blocked').length;
  const pendingCount = financeReceipts.filter(r => r.status === 'pending').length;
    ...
    <div className="space-y-6">
      <div className="page-header">
        <h1 className="page-title">
          <ModeText operator="Finance" engineer="Finance Visibility" />
        </h1>
        <p className="page-subtitle">
          <ModeText
            operator="Invoice, payment, and billing activity"
            engineer="Receipts filtered by finance-related action types"
          />
        </p>
      </div>

      <PurposeStrip
        operatorPurpose="Track all financial operations — invoices, payments, and billing events."
        engineerPurpose="Receipt audit trail filtered to finance-domain action types (invoice, payment, billing, charge, refund)."
        operatorAction="Review failed payments and pending invoices"
      ...
      <div className="flex flex-wrap gap-4">
        ...
        <div className="flex items-center gap-2 px-4 py-2 rounded-lg bg-surface-2 border border-border">
          ...
          <span className="text-sm">
            <span className="font-medium">{succeededCount}</span>
            <span className="text-muted-foreground ml-1">succeeded</span>
          </span>
        </div>
        {failedCount > 0 && (
      ...
      <Panel>
        {loading ? (
          <div className="loading-state">
            ...
            <span>Loading finance receipts...</span>
          </div>
        ) : error ? (
          <div className="empty-state">
            <AlertTriangle className="h-12 w-12 mx-auto mb-4 text-destructive/50" />
            <h3 className="text-lg font-medium mb-2">Failed to load finance data</h3>
          ...
          <div className="empty-state">
            ...
            <h3 className="text-lg font-medium mb-2">
              <ModeText operator="No finance activity" engineer="No finance receipts" />
            </h3>
            <p className="text-muted-foreground text-sm">
              <ModeText
                operator="Invoice and payment activity will appear here"
                engineer="No receipts match finance-related action type filters"
              />
            </p>
          </div>
        ) : (
        ...
        <SheetContent className="w-full sm:max-w-lg overflow-y-auto">
          {selectedReceipt && (
            <>
              <SheetHeader>
                <SheetTitle>
                  <ModeText operator="Finance Detail" engineer="Receipt Detail" />
                </SheetTitle>
              </SheetHeader>
